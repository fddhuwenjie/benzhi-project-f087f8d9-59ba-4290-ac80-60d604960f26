...
func New(s *store.Store) *Service {
	return &Service{store: s, locks: newPackageLocks(), views: map[string]*View{}}
}

func (s *Service) cachedView(id string) (*View, bool) {
	s.cacheMu.RLock()
...
func (s *Service) cacheView(id string, v *View) {
	s.cacheMu.Lock()
...
func (s *Service) save(v *View, action string) error {
	...
	v.AllowedActions = actions(v.Package.State)
	s.cacheView(v.Package.PackageID, v)
	return nil
}
