...
type Store struct {
	root         string
	mu           sync.Mutex
	packageCache map[string]cachedPackage
}
...
func New(root string) (*Store, error) {
	...
	return &Store{root: root, packageCache: map[string]cachedPackage{}}, nil
}
...
func (s *Store) SavePackage(p *domain.BroadcastPackage) error {
	...
	if err := writeAtomic(s.manifestPath(p.PackageID), []byte(d)); err != nil {
		return err
	}
	delete(s.packageCache, p.PackageID)
	return nil
}
...
func (s *Store) LoadPackage(id string) (*domain.BroadcastPackage, error) {
	...
	digest := string(m)
	cached, ok := s.packageCache[id]
	var b []byte
	if ok && cached.digest == digest {
		b = append([]byte(nil), cached.data...)
	} else {
		b, err = os.ReadFile(s.objectPath(digest))
		if err != nil {
			return nil, err
		}
		s.packageCache[id] = cachedPackage{digest: digest, data: append([]byte(nil), b...)}
	}
